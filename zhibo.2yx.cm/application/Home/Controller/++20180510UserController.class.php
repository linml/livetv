...
class UserController extends HomebaseController {
	...
	public function getCode(){
        ...
        $isexist=M("users")->where("user_login='{$mobile}' and user_type='2'")->find();
        if($type=='forget'){
            if(!$isexist){
                echo $_GET['callback']."({'errno':1003,'data':{},'errmsg':'该账号尚未注册，请注册'})";
                exit;
            }
        }else{
            if($isexist){
                echo $_GET['callback']."({'errno':1003,'data':{},'errmsg':'该账号已注册，请登录'})";
                exit;
            } 
        }
...
/* 	$user_login!=$_SESSION['mobile'] */
	...
	public function qqCallback()
	{
        ...
        //file_put_contents('./qqCallback.txt',date('Y-m-d H:i:s').' 提交参数信息 :'.json_encode($arr)."\r\n",FILE_APPEND);
		//得到 用户资料
		$users=M("users");
		$where=array(
			'openid'	=>	$openid,
		...
		{	
			if($openid!="")
			{
				...
				$data=array(
					...
					'user_login'	=> $user_login, 
					'user_pass'		=>$user_pass,
					'user_nicename'	=> checkNull($arr['nickname']),
					'sex'=> $sex,
			...
			{	
				if($user_message['id']!="")
				{
					...
					$data=array(
						...
						'user_login'	=> $user_login, 
						'user_pass'		=>$user_pass,
						'user_nicename'	=> checkNull($user_message['screen_name']),
						'sex'=>'0',
