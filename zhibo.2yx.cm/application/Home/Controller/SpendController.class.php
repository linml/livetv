...
class SpendController extends HomebaseController {
	...
	public function sendGift(){
		...
		if($configpri['family_switch']==1){
			$users_family= M("users_family")->field("familyid")->where("uid={$touid} and state=2")->find();
			...
			if($users_family){
				...
				/* 主播 */
				$liveuserinfo=$User->field('divide_family')->where("id='{$touid}'")->find();	
				...
				if($family_total){
					...
					M("family_profit")->add(array("uid"=>$touid,"time"=>$time,"addtime"=>$addtime,"profit"=>$family_total,"profit_anthor"=>$anthor_total,"total"=>$total,"familyid"=>$users_family['familyid']));
		...
		$result=array("uid"=>(int)$uid,"giftid"=>(int)$giftid,"giftcount"=>(int)$giftcount,"totalcoin"=>$total,"giftname"=>$giftinfo['giftname'],"gifticon"=>get_upload_path($giftinfo['gifticon']),"level"=>$level,"coin"=>$userinfo2['coin'],"votestotal"=>$userinfo3['votestotal']);
	...
	public function roomCharge()
	{
		...
		/* 家族 */
		if($configpri['family_switch']==1){
			$users_family=M("users_family")->field("familyid")->where("uid={$liveuid} and state=2")->find();
			...
			if($users_family){
				...
				/* 主播 */
				$liveuserinfo=M("users")->field("divide_family")->where("id={$liveuid}")->find();	
