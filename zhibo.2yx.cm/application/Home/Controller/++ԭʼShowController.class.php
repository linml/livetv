  ...
  //首页
	...
	*/
    ...
    function stopRoom(){
        ...
        $info=$Users_Live
                ->field("uid,showid,starttime,title,province,city,stream,lng,lat,type,type_val")
                ->where("uid='{$uid}' and stream='{$stream}' and islive='1'")
                ->find();
        if($info){
            $Users_Live->where("uid='{$uid}'")->delete();
            $nowtime=time();
