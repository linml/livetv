...
//		$firstLive = "rtmp://live.hkstv.hk.lxdns.com/live/hks";
		$firstLive = "http://29865.liveplay.myqcloud.com/live/29865_21010_1534923673.flv";
		...
		$hot=M("users_live l")
					...
					->select();
		 foreach($hot as $k=>$vi){
			$nums=$redis->hlen('userlist_'.$vi['stream']);
		...
		foreach($live as $k=>$vo){
			$nums=$redis->hlen('userlist_'.$vo['stream']);
	...
	function test(){
		$url1=PrivateKey_ali("rtmp","123",1);
		$url2=PrivateKey_ali("rtmp","123",0);
		$url4=PrivateKey_ali("rtmp","123",3);
		$url3=PrivateKey_ali("http","123.m3u8",0);
		$rs=array(
			'push'=>$url1,
			'pull'=>$url2,
			'pull2'=>$url3,
