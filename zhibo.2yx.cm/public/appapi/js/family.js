	...
	function upload(index)
	{
		...
		if(window.addEventListener){ 
			iptt.addEventListener('change',function(){
					...
					/*   var arr_img=new Array("/public/appapi/images/family/identity_face.jpg","/public/appapi/images/family/identity_back.jpg","/public/appapi/images/family/identity_handle.jpg");
...
$(function(){ 
	...
	$(".home .edit_divide").on("click",function(){
		...
		layer.prompt({title: '家族默认分成(%)', formType: 0,value:divide}, function(pass, index){
			if(hasEmoji(pass)){
				layer.msg("不能含有表情");
				return !1;
			}
	...
	$(".member .user-list ul li .action span.no").on("click",function(){
		...
		layer.confirm('要把成员'+name+'踢出家族吗？',{}, function(index){
			layer.close(index);
