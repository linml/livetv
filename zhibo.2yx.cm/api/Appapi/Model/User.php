	...
	public function getUserHome($uid,$touid){
		...
		if($uid==$touid){
			...
			$live=DI()->notorm->users_live
					->select("uid,avatar,avatar_thumb,user_nicename,title,city,stream,pull,type,thumb,game_action,goodnum,isvideo,anyway")
			...
			if(!$live){
				...
				$live['game']=$this->game_action[$live['game_action']];
			}	
