...
class Api_Live extends Api_Common {
	...
	public function getRules() {
		return array(
			...
			'getAuction' => array(
                'id' => array('name' => 'id', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '拍品ID'),
            ),
			
			'setAuction' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '会员ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'min' => 1, 'desc' => '会员token'),
				'auctionid' => array('name' => 'auctionid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '拍品ID'),
            ),
	 ...
	 * @return string info[0].agorakitid 声网ID 
	...
	public function createRoom() {
		...
		$anyway=$this->checkNull($this->anyway);
		...
		$info['auction_switch']=$configpri['auction_switch'];
		...
		$info['game_banker_coin']=$this->NumberFormat(10000000);
	...
	public function stopRoom() { 
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		//file_put_contents('./stopRoom.txt',date('Y-m-d H:i:s').' 提交参数信息 $_REQUEST:'.json_encode($_REQUEST)."\r\n",FILE_APPEND);
		$uid = $this->uid;
		$token=$this->checkNull($this->token);
		$stream=$this->checkNull($this->stream);
		...
		if(!$isexist){
			...
			if($checkToken==700){
				$rs['code'] = $checkToken;
				$rs['msg'] = '您的登陆状态失效，请重新登陆！';
                //file_put_contents('./stopRoom.txt',date('Y-m-d H:i:s').' 提交参数信息 rs1:'.json_encode($rs)."\r\n",FILE_APPEND);
				return $rs;
			}
		...
		$rs['info'][0]['msg']='关播成功';
        //file_put_contents('./stopRoom.txt',date('Y-m-d H:i:s').' 提交参数信息 rs2:'.json_encode($rs)."\r\n",FILE_APPEND);
		return $rs;
	}	
	 ...
	 * @return string info[0].game_banker_limit 上庄限额 
	 * @return string info[0].agorakitid 声网ID 
	 * @return object info[0].auction 竞拍
	 * @return string info[0].auction.isauction 竞拍是否进行
	 * @return string info[0].auction.id 竞拍ID
	 * @return string info[0].auction.title 名称
	 * @return string info[0].auction.thumb 图片
	 * @return string info[0].auction.price_start 起拍价
	 * @return string info[0].auction.price_bond 保证金
	 * @return string info[0].auction.price_fare 加价幅度
	 * @return string info[0].auction.bid_price 竞拍金额
	 * @return string info[0].auction.bid_uid 竞拍用户ID
	 * @return string info[0].auction.user_nicename 竞拍用户昵称
	 * @return string info[0].auction.avatar 竞拍用户头像
	 * @return string info[0].auction.long 剩余竞拍时间（秒）
	 * @return string info[0].auction.pay_long 支付时长
	 * @return object info[0].vip 用户VIP信息
	 * @return string info[0].vip.type VIP类型，0表示无VIP，1表示普通VIP，2表示至尊VIP
	 * @return object info[0].liang 用户靓号信息
	 * @return string info[0].liang.name 号码，0表示无靓号
	...
	public function enterRoom() {
	    ...
	    $info=array(
			...
			'agorakitid'=>$configpri['agorakitid'],
		...
		/* 竞拍 */
		$auction = $domain->checkAuction($liveuid);
     ...
     * @return int code 操作码，0表示成功
     * @return array info 
	 * @return object info[0].task 任务进度
	 * @return string info[0].task.complete 完成任务总数
	 * @return string info[0].task.task 任务总数
	 * @return string info[0].task.tip 显示进度
     * @return string msg 提示信息
     */
     ...
     * @return int code 操作码，0表示成功
     * @return array info 
	 * @return object info[0].bid_price 竞拍金额
     * @return string msg 提示信息
     */
	...
	public function setAuction(){
		...
		$file = API_ROOT.'/public/setauction.txt';
		$fp = fopen($file, 'a');
		if(flock($fp, LOCK_EX|LOCK_NB)){            // 取得独占锁
			$domain = new Domain_Live();
			$info = $domain->setAuction($uid,$auctionid);
		 	flock($fp, LOCK_UN);                    // 解锁
		 	fclose($fp);
			if($info['code']!=0){
				$rs['code']=$info['code'];
     ...
     * @return int code 操作码，0表示成功
     * @return array info 
	 * @return object info[0].coin 用户余额
     * @return string msg 提示信息
     */
	...
	public function setBond(){
		...
		$info = $domain->setBond($uid,$auctionid);
		if($info==1001){
			$rs['code']=$info;
			$rs['msg']='竞拍信息不存在';
			return $rs;
     ...
     * @return int code 操作码，0表示成功
     * @return array info 
	 * @return object info[0].bid_price 竞拍价格
	 * @return object info[0].bid_uid 竞拍用户ID
	 * @return object info[0].user_nicename 竞拍用户昵称
	 * @return object info[0].avatar 竞拍用户头像
     * @return string msg 提示信息
     */
	...
	public function auctionEnd(){
		...
		$info = $domain->auctionEnd($uid,$auctionid);
		if($info==1001){
			$rs['code']=$info;
			$rs['msg']='竞拍信息不存在';
			return $rs;
     ...
     * @return array info 
	 * @return object info[0].coin 用户余额
	 * @return object info[0].level 用户等级
	...
	public function setBidPrice(){
		...
		$info = $domain->setBidPrice($uid,$auctionid);
		if($info==1001){
			$rs['code']=$info;
			$rs['msg']='竞拍信息不存在';
			return $rs;
