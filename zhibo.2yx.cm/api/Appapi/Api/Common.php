...
class Api_Common extends PhalApi_Api {
	...
	/* 设置缓存 可自定义时间*/
	protected function setcaches($key,$info,$time=0){
	...
	protected function sendCode($mobile,$code){
		...
		if($gets['SubmitResult']['code']==2){
            ...
            $domain = new Domain_Common();
            $domain->setSendcode(array('type'=>'1','account'=>$mobile,'content'=>$content));
	...
	*/
	...
	/* 检测用户是否存在 */
	protected function checkUser($where){
		$domain = new Domain_Common();
		$rs = $domain->checkUser($where);
		return $rs;
